// Package rtf parse and manipulate RTF documents
package rtf
...
// AsText reads a .rtf file and returns the content as a string
func AsText(filename string) (string, error) {
        content, err := ioutil.ReadFile(filename)
        if err != nil {
                return "", err
        }
...
func FromBytes(data []byte) (string, error) {
	if (bytes.Equal(data[:6], []byte("{\\rtf1"))) {
		return BytesToStr(data)
	} else {
		return "", errors.New("rtf: Invalid file type")
	}
}

func readControl(r peekingReader.Reader, s *lls.Stack, text *bytes.Buffer) error {
	...
	for {
		...
		case b >= '0' && b <= '9' || b == '-':
			if numStart == -1 {
				...
				return "", -1, errors.New("rtf: Unexpected control sequence. Cannot begin with digit")
			}
			buf.WriteByte(b)
			r.ReadByte() // consume valid digit
